...
def send_via_email(msg, subject):
    ...
    smtpserver = smtplib.SMTP("ssl0.ovh.net",465)
#    smtpserver.ehlo()
    smtpserver.starttls()
    smtpserver.ehlo
...
def match_description_rule(trello_db, desc):
    #Finds a description rule matching the given product description.Returns the rule if it exists, otherwise returns None."""
